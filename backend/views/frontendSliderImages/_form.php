...
<?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm', array(
	'id'=>'frontend-slider-images-form',
	...
	<div class="row">
		<label for="Image_file"><?php echo Yii::t('label','Image'); ?></label>
				...
				<?php
					$this->widget('yiiwheels.widgets.fileupload.WhBasicFileUpload',
						array(
							...
							'attribute' => 'file',
							'model' => $image,						
							'uploadAction' => $this->createUrl('upload',array('svar'=>'FrontendSliderImg')),
							'pluginOptions' => array(
								'dataType' => 'json',
								...
								'done' => 'js:function(e, data){
									jQuery.each(data.result.files, function(i, file){
										jQuery(\'<div class="row"><div class="span3">\'+file.name+\'</div><div class="span2"><a href="\'+file.deleteUrl+\'"&svar=FrontendSliderImg class="deleteupload btn btn-danger"><i class="icon-trash"></i> '.Yii::t('label','Delete').'</a></div></div>\').appendTo("#bfiles");
									});
								}',
			...
			<?php if(isset($userImages)){
				...
				<div class="row"><div class="span3"><?php echo $image["name"]; ?></div><div class="span2"><a href="<?php echo $this->createUrl( "upload", array("_method" => "delete","file" => $image["filename"],"svar" => "FrontendSliderImg") );?>" class="deleteupload btn btn-danger"><i class="icon-trash"></i> <?php echo Yii::t('label','Delete'); ?></a></div></div>
			<?php }
			}
			?>
			</div>
