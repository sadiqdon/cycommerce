...
class TaxClassController extends CrudController
{
	public $modelName = 'TaxClass';
	...
	public function actionUpdate($id) {
		...
		$description = $model->taxClassDescriptions[0];
		$this->performAjaxValidation(array($model,$description), 'tax-class-form');
		
		if (isset($_POST[$this->modelName])) {
			$model->setAttributes($_POST[$this->modelName]);
			$description->setAttributes($_POST[$this->modelName.'Description']);
			$suc = Yii::t('info','TaxClass was successfully updated');
			$err = Yii::t('info','Could not update TaxClass');
			if ($model->validate() && $description->validate()){
				if ($model->save()) {
					$description->tax_class_id = $model->id;
	...
	public function actionDelete($id) {
			parent::delete($id, $this->modelName, array('taxClassDescriptions'));
	}
	
	public function actionBatchDelete() {
			parent::batchDelete($this->modelName, array('taxClassDescriptions'));
	}
	...
	public function actionIndex() {
		...
		$attr['name'] = array('asc' => 'taxClassDescriptions.name ASC', 'desc' => 'taxClassDescriptions.name DESC');
