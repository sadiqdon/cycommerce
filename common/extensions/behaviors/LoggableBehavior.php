...
class LoggableBehavior extends CActiveRecordBehavior{
	...
	*/
	...
	public function leaveTrail($action, $name = null, $value = null, $old_value = null){
		...
		$log->new_value = $value;
		//$log->host_add = (gethostbyaddr(Yii::app()->request->getUserHostAddress()) != false) ? gethostbyaddr(Yii::app()->request->getUserHostAddress()) : Yii::app()->request->getUserHostAddress();
		$log->host_add = Yii::app()->request->getUserHostAddress();
		$log->action	= $action;
