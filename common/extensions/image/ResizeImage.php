 ...
 *
 * $Id: ResizeImage.php 3809 2008-12-18 12:48:41Z OscarB $
 *
 * @package    ResizeImage
 * @author     Kohana Team
...
class ResizeImage {
	...
	/**
	 * Creates a new ResizeImage instance and returns it.
	 *
	...
	{
		return new ResizeImage($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! is_file($image))
			throw new CException('image file('.$image.') not found');
		...
		if ( ! isset(ResizeImage::$allowed_types[$image_info[2]]))
			throw new CException('image type not allowed');

		// ResizeImage has been validated, load it
		$this->image = array
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => ResizeImage::$allowed_types[$image_info[2]],
	 ...
	 * wish to use height as master dim, set $image->master_dim = ResizeImage::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: ResizeImage::NONE, ResizeImage::AUTO, ResizeImage::WIDTH, ResizeImage::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== ResizeImage::NONE AND
				    $value !== ResizeImage::AUTO AND
				    $value !== ResizeImage::WIDTH AND
				    $value !== ResizeImage::HEIGHT)
					return FALSE;
			break;
		}
