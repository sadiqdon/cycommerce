...
class TbApi extends CApplicationComponent
{
    ...
    public function registerCoreCss($url = null)
    {
        ...
        if ($url === null) {
			...
			$filename2 = YII_DEBUG ? 'font-awesome.css' : 'font-awesome.min.css';
			$cs->registerCssFile($this->getAssetsUrl().'/css/'.$filename2);
			$cs->registerCssFile($this->getAssetsUrl().'/css/font-awesome-ie7.min.css');
        }else{
			$cs->registerCssFile($url);
		}
    }
     ...
     */
	 // changed default position to CClientScript::POS_HEAD
