...
class Editable extends CWidget
{
    ...
    */
    ...
    public function registerClientScript()
    {
        ...
        if($this->liveTarget) {
            $script .= "\n jQuery('body').on('ajaxUpdate.editable', function(e){ if(e.target.id == '".$this->liveTarget."') yiiEditable(); });";
    ...
    public function registerAssets()
    {
        ...
        if($form === EditableConfig::FORM_BOOTSTRAP) {
            if (($bootstrap = yii::app()->getComponent('bootstrap'))) {
				//Disable bootstrap because its already registered
            ...
            $assetsUrl = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.bootstrap-editable'));
        ...
        // jqueryui
            ...
            $assetsUrl = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.jqueryui-editable'));
            $js = 'jqueryui-editable.js';
            $css = 'jqueryui-editable.css';
        // plain jQuery
        } else {
            $assetsUrl = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.jquery-editable'));
            ...
            if($mode === EditableConfig::POPUP) {
                $poshytipUrl = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.poshytip'));
        ...
        if($this->type == 'combodate') {
            $momentUrl = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.moment'));
            $cs->registerScriptFile($momentUrl.'/moment.min.js');          
        }
        
        //include select2 lib for select2 type
        if($this->type == 'select2') {
            $select2Url = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.select2'));
        ...
        if($this->type == 'datetime') {
            $url = $am->publish(Yii::getPathOfAlias('common.extensions.editable.assets.bootstrap-datetimepicker'));
    ...
    {
		echo CHtml::tag('input', $this->inputField, false, true);
	}
    ...
    */
    ...
    {
        $trigger = 'jQuery("#'.$widget->id.'").trigger("ajaxUpdate.editable");';
